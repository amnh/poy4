...
\begin{command}{transform}{transformcommand}
	 ...
	 \argumentdefinition{breakinv\_to\_custom}{}
        ...
        \argumentdefinition{seq\_to\_chrom}{\obligatory{([argument list])}}   
           ...
           {seqtochrom}
            ...
            (\poyargument{seed\_length}, \poyargument{min\_loci\_len}, and \poyargument{min\_rearranged\_len}); specify the cost
            ...
            (i.e. \poyargument{chrom\_inversion} or \poyargument{chrom\_breakpoint}, and \poyargument{chrom\_indel}); 
	...
	\begin{description}
		     ...
		     \argumentdefinition{seed\_length}{\obligatory{\poyint}}
                        ...
                        may significantly increase. The optimal \poyargument{seed\_length} 
		...
		\end{figure}
      		     ...
      		     {Calculates the breakpoint distance~\cite{blanchetteetal1997}
                       ...
                       {Calculates the inversion distance~\cite{hanenhalliandpevzner1995}
                        ...
                        specified by the integer value. The inversion distance
                        takes in consideration rearrangements an inversions.  For further discussion on 
                        how breakpoint distance is calculated see the argument \poyargument {locus\_inversion}.  
                        This argument \emph{cannot} be used in conjunction with \poyargument{chrom\_breakpoint}.} 
                        {chrominversion}
	...
	\begin{poyexamples} 
		...
		and is treated as a circular chromosome.}
          	...
          	\poyexample{transform(seq\_to\_chrom:(locus\_indel:(50, 1.0)))} 
          	{All applicable (\emph{i.e.} sequence) data are transformed into chromosome
          	data and the locus-level gap opening cost is set at \texttt{50} with a gap extension cost at \texttt{1.0}.}
              
          	\poyexample{read (chromosome:("mito")) transform((all,
                   dynamic\_pam:\\(locus\_breakpoint:10, min\_rearrangement\_len:60, median:1, \\circular:false)))} 
             	{This example shows a file read (``mito'') containing mitochondrial chromosome sequences that has been transformed 
		to set the locus rearrangement (breakpoint) cost at \texttt{10}, and only strings of \texttt{60} or more nucleotides are considered in determining possible
                   rearrangements between identified seeds.  The chromosome data are treated as linear and only a single set of median rearrangements are stored.}
