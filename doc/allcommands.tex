\begin{command}{build}{buildcommand}
	...
	\begin{arguments}
        ...
        \argumentdefinition{branch\_and\_bound}{\optional{\poyfloat}}
            {Instead of building a tree the default heuristic method, calculate
            the exact solution using the branch and bound algorithm, and return
            as many trees as the \poyargument{trees} argument specify, 
            within the optional threshold value of the argument. By default only
            one tree which is the optimal solution will be kept in the result.}
            {branchandbound}
        ...
        \argumentdefinition{all}{}
            {Turns off all preference strategies to make a join, simply try all possible join
            positions for all terminals added.}
            {allbuild}
...
\begin{command}{rename}{}
	...
	\syntax{\obligatory{argument list}}
...
\begin{command}{report}{}
	...
	\begin{argumentgroup}{Terminals and characters}
		...
		\begin{argumentgroup}{Other arguments}
	     ...
	     {Applies a user-defined stylesheet to the xml output. The first string is
...
\begin{command}{set}{}
	...
	\begin{statement}
	  ...
	  as produced by the diagnosis of the same tree. However, once the cost is computed
	  by the diagnosis, subsequent rediagnosing of the same tree must produce the same
	  cost.
	\end{statement}
