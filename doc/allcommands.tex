...
\begin{command}{cd}{cd}
	...
	\begin{poyexamples}
		...
		\poyexample{cd ("/Users/username/docs/poyfiles")}
            ...
            PC the path will the forward slashes will be replaced with backslash characters is the path).\\
...
\begin{command}{search}{}
	...
	\begin{poydescription}
            ...
            maximum allowed memory consumption, minimum number of times the
            shortest tree is found, and an expected cost for the shortest tree.  When executing
             \poycommand{search} using parallel processing trees are exchanged upon the
              ...
              command in a parallel environment consists of implementing several sequential
               \poycommand{search} commands that will add the pool of selected
	...
	\begin{arguments}
        ...
        memory allocated for the search per processor. \poy \emph{attempts} to consume memory within
        the specified limit, but it may surpass it in certain operations. The lident value expresses the 
    ...
    the search will be aborted after one hour.}}
        ...
        \begin{statement}
                Note that in order to maximize computational efficiency when using \poycommand{search} 
                in parallel processing environments the \poyargument{hits} argument has been removed from
                the valid syntax of the command.  Because the number of trees evaluated in search is \poycommand{search} 
                is multiplicative such that it is doubled in the \poycommand{fuse} stage during each iteration that this 
                command performs prior to its user defined \poyargument{max\_time}, a strategy in which many 
                successive \poycommand{search} commands are executed for relatively short amounts of time is more 
                exhaustive than a single \poycommand{search} executed for a relatively long amount of time.  During each 
                successive \poycommand{search}, in addition to the trees generated in the \poycommand{build} stage, the set of 
                best unique trees selected from the previous search round is applied to the current \poycommand{search} at 
                the \poycommand{fuse} stage and the set of best trees from each processor will be exchanged at the command's completion.
                \end{statement}
	...
	\poyexample {For Parallel Implementation of \poycommand{search}}
            ...
            fusings as possible within the specified time of 16 hours (2 hours allocated
             for each successive iteration of the command).  Trees are exchanged among
             processors at the end of each \poycommand{search} and the best unique trees
              are then selected and applied to the following  \poycommand{search} command.}
