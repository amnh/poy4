...
\begin{command}{search}{}
    ...
    the search will be aborted after one hour.}}
        ...
        \begin{statement}
                ...
                include the current best trees found ammong all the processes is
                desirable to improve the potential of tree fusing.

                To guarantee that separate processes
                exchange trees and seed each other's fuse step, it
                is advisable to use few successive search commands
                when executing the program in parallel, to ensure
                proper tree exchange between different processes,
                increasing their probability of finding better trees
	...
	\poyexample {For Parallel Implementation of \poycommand{search}}
            ...
            fusings as possible within the specified total time of 16 hours.  Trees are exchanged among
             processors at the end of each \poycommand{search} and the best unique trees
              are then selected and included in the following \poycommand{search} command.}
            \end{poyexamples}
	...
	\begin{arguments}
        ...
        \begin{argumentgroup}{Cost calculation}
            ...
            \argumentdefinition{exhaustive\_do}{}
                ...
                \argumentdefinition{iterative}{\obligatory{\poybool}}
                {Applies the Iterative Pass optimization for the tree cost
                calculations. There are two forms of iterative pass: if the
                argument value is true (the default), then a complete three
                dimensional alignment is computed, as described for POY 3.0 in~\cite{wheeler2003a}. 
                Otherwise, if the argument value is false, then the iterations
                approximate the three dimensional alignment using pairwise
                alignments. The later is much faster than the former, but the
                former is default for compatibility reasons.
                If the argument value is true, this method improves the tree
                cost estimation but at the expense of a tremendous execution
                time. When false the execution time footprint is much smaller. The iterative argument 
                can only be applied to nucleotide sequence characters.
                A hueristic strategy is to apply  \poyargument{iterative} at the 
