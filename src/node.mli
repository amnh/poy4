...
val to_formatter_subtree :
    ChromCS.IntSet.t * ChromCS.IntSet.t -> Tags.attributes ->
  Data.d ->
  node_data ->
  int ->
...
(** [to_single p n] returns a node that contains per character a single state
* which is closest to [p] among those available in [n]. Useful for tree length
* verification. *)
val to_single : node_data -> node_data -> node_data

(** [to_single_root n] is equivalent to [to_single n n]. *)
val to_single_root : node_data -> node_data
