type datatype = DStandard | Dna | Rna | Nucleotide | Protein | Continuous
type item =
    Min
  | Max
  | Median
  ...
  | SampleSize
  | States
type statesformat = StatesPresent | Individuals | Count | Frequency
...
type standard_item =
    Code of (string * charset list)
  | IName of (string * charset list)
...
type block =
  ...
  | Distances of
      ((bool * string * string) option * format_options list * string list *
       string)
  | Ignore of string
  | Unaligned of unalg_data
  | Trees of ((string * string) list * string list)
  | Notes of
      ((set_pair list * source * string) option *
       (set_pair list * pictureformat option * pictureencoding option *
        source * string)
       option)
