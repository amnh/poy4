...
module MakeNormal (Node : NodeSig.S) (Edge : Edge.EdgeSig with type n = Node.n) 
    ...
    * original addition sequence as generated by [b ()].*)
    let max_n_wagner tabu_mgr data_generator cg n =
    ...
    let max_n_dg_p_wagner tabu_mgr data_generator cg n p : phylogeny Sexpr.t =
        match p, n with
        | 0, _ -> `Empty
        | 1, 1 -> 
        ...
        | 1, n -> 
                let st = Status.create "Building Wagner Tree" None "" in
                let res = max_n_wagner tabu_mgr data_generator cg n in
                Status.finished st;
                res
        | p, _ ->
              let builder cnt acc =
                  let next = max_n_wagner tabu_mgr data_generator cg n in
                  next :: acc
              in
    ...
    * function [b ()]. *)
    let max_n_randomized_p_wagner tabu_mgr cg data n p =
        let data_generator () = randomize data in
        max_n_dg_p_wagner tabu_mgr data_generator cg n p

    let max_n_mst_p_wagner tabu_mgr data cg nodes n p =
        let mst = mst data nodes in
        max_n_dg_p_wagner tabu_mgr mst cg n p

    let max_n_distances_p_wagner tabu_mgr cg nodes n p =
        let dord = distances_ordered nodes in
        max_n_dg_p_wagner tabu_mgr dord cg n p
        ...
        | `Build (n, build_meth, lst) ->
                ...
                if n = 0 then trees
                    ...
                    | `Wagner_Rnd (max_n, _, lst, tabu_mgr) ->
                            ...
                            let res = 
                                max_n_randomized_p_wagner tabu_mgr cg 
                                new_nodes max_n n
                            in
                            Sexpr.of_list (Sexpr.to_list res)
                    | `Wagner_Ordered (max_n, keep_method, lst, tabu_mgr) ->
                            let tabu_mgr = pick_tabu_manager tabu_mgr in
                            ...
                            let () =
                                ...
                                status in
                            let hd = 
                                max_n_wagner tabu_mgr (fun () -> new_nodes) cg 
                                max_n 
                            in
                            let () =
                                ...
                                status in
                            let tl = 
                                max_n_randomized_p_wagner tabu_mgr cg new_nodes 
                                max_n (n - 1)
                            in
                            let () = Status.finished status in
                            Sexpr.of_list (Sexpr.to_list (`Set [hd; tl]))
                    | `Wagner_Distances (max_n, keep_method, lst, tabu_mgr) ->
                            let tabu_mgr = pick_tabu_manager tabu_mgr in
                            let status = 
                            ...
                            let () =
                                ...
                                status in
                            let lst = 
                                max_n_distances_p_wagner tabu_mgr cg new_nodes 
                                max_n n 
                            in
                            let () = Status.finished status in
                            Sexpr.of_list (Sexpr.to_list lst)
                    | `Wagner_Mst (max_n, keep_method, lst, tabu_mgr) ->
                            let tabu_mgr = pick_tabu_manager tabu_mgr in
                            let status = 
                            ...
                            let () =
                                ...
                                status in
                            let lst = 
                                max_n_mst_p_wagner tabu_mgr data cg new_nodes 
                                max_n n 
                            in
                            let () = Status.finished status in
                            Sexpr.of_list (Sexpr.to_list lst)
