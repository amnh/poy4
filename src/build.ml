...
let () = SadmanOutput.register "Build" "$Revision: 1803 $"
...
let rec get_transformations (meth : Methods.build) : Methods.transform list =
    ...
    | `Build (n, build_meth, trans) ->
            ...
            | `Wagner_Distances (_, _, trans)
            | `Wagner_Mst (_, _, trans) 
            | `Wagner_Rnd (_, _, trans) 
            | `Wagner_Ordered (_, _, trans)
            | `Build_Random (_, _, trans) -> trans
            | `Prebuilt _ -> [])) []
    | `Build_Random (_, _, trans) -> 
            List.fold_right remove_exact trans []
...
module MakeNormal (Node : NodeSig.S) (Edge : Edge.EdgeSig with type n = Node.n) 
    ...
    let single_wagner_search_manager = new Queues.wagner_srch_mgr 1 0.0
    ...
    let single_wagner wmgr cg data = 
        ...
        | (hd, _) :: _ -> 
                `Single (TreeOps.clear_internals (PtreeSearch.uppass hd))
    ...
    * original addition sequence as generated by [b ()].*)
    let max_n_wagner data_generator cg n =
        let wmgr = new Queues.wagner_srch_mgr n 0.0 in
        ...
        BuildTabus.wagner_tabu)#results in
        `Set (List.map (fun (x, _) -> 
            `Single (TreeOps.clear_internals (TreeOps.uppass x))) res)
    ...
    let mst data nodes =
        ...
        let mst = Mst.kruskal distance_fn codes in
        let do_mst () =
            let data = Mst.randomized_dfs_traversal mst in
            List.map (fun x ->
                List.find (fun y -> 
    ...
    let max_n_dg_p_wagner data_generator cg n p : phylogeny Sexpr.t =
        match p, n with
        | 0, _ -> `Empty
        | 1, 1 -> 
        ...
        | 1, n -> 
                let st = Status.create "Building Wagner Tree" None "" in
                let res = max_n_wagner data_generator cg n in
                Status.finished st;
                res
        | p, _ ->
              let builder cnt acc =
                  let next = max_n_wagner data_generator cg n in
                  next :: acc
              in
    ...
    * function [b ()]. *)
    let max_n_randomized_p_wagner cg data n p =
        let data_generator () = randomize data in
        max_n_dg_p_wagner data_generator cg n p

    let max_n_mst_p_wagner data cg nodes n p =
        let mst = mst data nodes in
        max_n_dg_p_wagner mst cg n p

    let max_n_distances_p_wagner cg nodes n p =
        let dord = distances_ordered nodes in
        max_n_dg_p_wagner dord cg n p
    ...
    let prebuilt trees data = 
        ...
        let constructor (cnt, lst) x =
            ...
            let t = 
                PtreeSearch.uppass (PtreeSearch.downpass (PtreeSearch.convert_to x data))
            in
            let t = TreeOps.clear_internals t in
            cnt + 1, (`Single t) :: lst
        in
        let res = 
            `Set 
                    ...
                    | `Wagner_Distances (max_n, keep_method, lst) ->
                          ...
                          let () =
                              Status.full_report ~msg:"Building trees"
                                  status in
                          let lst = max_n_distances_p_wagner cg new_nodes max_n n in
                          let () = Status.finished status in
                          Sexpr.of_list (Sexpr.to_list lst)
                    | `Wagner_Mst (max_n, keep_method, lst) ->
                          let status = 
                          ...
                          let () =
                              Status.full_report ~msg:"Building trees"
                                  status in
                          let lst = max_n_mst_p_wagner data cg new_nodes max_n n in
                          let () = Status.finished status in
                          Sexpr.of_list (Sexpr.to_list lst)
